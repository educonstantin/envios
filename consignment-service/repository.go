...
func (repo *ConsignmentRepository) GetAll() ([]*pb.Consignment, error) {
	...
	// We the bind our consignments variable by passing it as an argument to .All().
	// That sets consignments to the result of the find query.
	// There's also a 'One()' funtion for single results.
	err := repo.collection().Find(nil).All(&consignments)
	return consignments, err
}

// Close closes the database session after each query has ran.
// Mgo create a 'master' session on start-up, it's then good practice
...
// as under the hood each session has its own database socket end error handling.
